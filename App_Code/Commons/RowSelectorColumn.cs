			...
			switch (itemType)
			{
				...
				case ListItemType.SelectedItem:
					...
					if (this.SelectionMode == ListSelectionMode.Multiple)
					{
						...
						selector.ID = "RowSelectorColumnSelector" + columnIndex;    //Peak 18/07/2008 เพิ่ม ColumnIndex เพื่อให้ใช้ Control ได้มากกว่า 1 Column ใน Grid;;
				...
				case ListItemType.Header:
					if (AllowSelectAll && this.SelectionMode == ListSelectionMode.Multiple)
					{
						...
						selectAllControl.ID = "RowSelectorColumnAllSelector" + columnIndex; //Peak 18/07/2008 เพิ่ม ColumnIndex เพื่อให้ใช้ Control ได้มากกว่า 1 Column ใน Grid;;
		...
		protected virtual void RegisterSelectAllScript()
		{
			if (this.Owner.Page != null)
			{
				if (!this.Owner.Page.IsClientScriptBlockRegistered("MetaBuilders_RowSelectorColumn Library"))
				{
					...
					using (StreamReader reader = new StreamReader(typeof(RowSelectorColumn).Assembly.GetManifestResourceStream("DataGridEx.UIControls.RowSelectorColumnScript.js")))
					{
						String script = "<script language='javascript'>\r\n<!--\r\n" + reader.ReadToEnd() + "\r\n//-->\r\n</script>";
						this.Owner.Page.RegisterClientScriptBlock("DataGridEx.UIControls.RowSelectorColumn", script);
					}
				}
			}
		}
