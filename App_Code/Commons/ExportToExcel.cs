...
namespace EB_Service.Commons.Excel
{
    # region Summary
    ...
    public class Export
    {
        public enum ExportFormat : int { CSV = 1, Excel = 2, Text = 3 }; // Export format enumeration			
        ...
        #region ExportDetails OverLoad : Type#4
        ...
        public void ExportDetails(GridView gvGridView, ExportFormat FormatType)
        {
            try
            {
                // Appending Headers
                ...
                gvGridView.HeaderStyle.Font.Bold = true;
                gvGridView.DataBind();
                ...
                if (FormatType == ExportFormat.CSV)
                    ...
                    response.ContentType = "application/x-msexcel";//"application/vnd.ms-excel";


                response.ContentEncoding = Encoding.GetEncoding("windows-874");
        ...
        private void Export_with_XSLT_Web(DataSet dsExport, string[] sHeaders, string[] sFileds, ExportFormat FormatType, string FileName)
        {
            try
            {
                ...
                {
                    response.ContentType = "application/text";//"application/vnd.text";
                    response.AppendHeader("content-disposition", "attachment; filename=" + FileName);

                    System.IO.StringWriter st = new StringWriter();
