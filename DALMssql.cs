...
namespace EB_Service.DAL
{
    public class DataCenter : UserInfo
    {
        ...
        public async Task<(bool success, string message, int afrows)> ExecuteBluk(string TableName, DataTable BlukData, bool transaction = false, IEnumerable<SqlParameter> param = null, bool DBlink = false)
        {
            (bool success, string message, int afrows) result;
            try
            {
...
#if DEBUG
                var afrows = await _ExecuteTestBulk(TableName, BlukData, transaction);
#else
                var afrows = await _ExecuteBulk(TableName, BlukData, transaction);
#endif
                if (afrows > 0) result = (true, "success", afrows);
                else result = (true, "The command was executed but 0 row affected.", afrows);
                //Utility.WriteLogResponse("Execute ", result.message.ToString());
        ...
        public async Task<int> _ExecuteBulk(string TableName, DataTable BulkData, bool transaction = false)
        {
            int afrows = 0;
            //m_userInfo = (UserInfo)HttpContext.Current.Session["UserInfo"];
            UserInfomation = m_userInfo;
            ...
            if (methodStatus)
            {
                ...
                () =>
                {
                    ...
                    try
                    {
                        SqlBulkCopy sqlBulk = new SqlBulkCopy(SqlCon,SqlBulkCopyOptions.TableLock,Sqltr);
                        sqlBulk.DestinationTableName = TableName;
                        sqlBulk.BatchSize = 1000;
                        sqlBulk.BulkCopyTimeout = 0;
                        sqlBulk.WriteToServerAsync(BulkData);
        ...
        public async Task<int> _ExecuteTestBulk(string TableName, DataTable BulkData, bool transaction = false) // for test on localhost
        {
            int afrows = 0;
            if (SqlCon != null && SqlCon.State != ConnectionState.Open)
                SqlCon.Open();
