...
function ShowTooltipDropdownList(drw) {


    var intSelectedIndex = drw.selectedIndex;
    //check if there is selected item or not
    if (intSelectedIndex != -1) {
        var strText = drw.options[intSelectedIndex].text;
        drw.title = strText;
    }
    for (i = 0; i < drw.options.length; i++) {
        drw.options[i].title = drw.options[i].text;
